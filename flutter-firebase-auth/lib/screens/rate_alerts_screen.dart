...
class _RateAlertsScreenState extends State<RateAlertsScreen> {
  ...
  @override
  void dispose() {
    super.dispose(); // No custom disposal needed now
  }

  Future<void> _fetchAllRates() async {
    const String apiKey = '03a85ceec4f1b8e243e2c87e';

    try {
      final futures = currencies.map((base) async {
        final url = Uri.parse(
        ...
        if (response.statusCode == 200) {
          ...
          if (data['result'] == 'success' &&
            ...
            return MapEntry(base, filteredRates);
          } else {
      ...
      setState(() {
        ...
        for (final entry in results) {
          if (entry != null) {
            _allRates[entry.key] = entry.value;
          }
        }
        _isLoading = false;
      });
      ...
      body: PageWithLoader(
        ...
        child: _allRates.isEmpty
            ...
            : ListView(
                children: _allRates.entries
                    .map((entry) =>
                        _buildRateSection(entry.key, entry.value))
                    .toList(),
              ),
      ),
    );
